...
const Thick3DTile = ({ children, thickness = 20, ...props }: any) => {
    return (
        <motion.div {...props} style={{ transformStyle: 'preserve-3d', ...props.style }}>
            ...
            <div
                className="absolute inset-0 rounded-[28px] md:rounded-[40px] bg-[#020304] shadow-[0_35px_80px_rgba(0,0,0,1)]"
                style={{ transform: `translateZ(-${thickness}px)`, filter: 'blur(6px)' }}
            />
            ...
            {Array.from({ length: thickness }).map((_, i) => (
                <div
                    key={i}
                    className="absolute inset-0 rounded-[28px] md:rounded-[40px] bg-[#090b0e]"
                    style={{
            ...
            {/* Top Surface Layer */}
            <div
                className="absolute inset-0 rounded-[28px] md:rounded-[40px] overflow-hidden bg-gradient-to-br from-[#2a374b] via-[#0f141a] to-[#040608] flex items-center justify-center border-t border-l border-white/20"
                style={{
...
export default function HeroFloatingTiles() {
    ...
    // 사용자의 피드백에 따라 타일 크기를 다소 작게(Scale Down) 조절했습니다.
    const tiles = [
        {
