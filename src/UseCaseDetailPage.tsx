                            ...
                            <section key={section.id} id={section.id} className="flex flex-col scroll-mt-32">
                                ...
                                {section.id === 'summary' ? (
                                    <div className="p-6 rounded-[20px] bg-brand-primary/10 border border-brand-primary/20">
                                        ...
                                        <div className="text-text-secondary text-[16px] leading-relaxed break-keep font-medium">
                                            {section.content}
                                        </div>
                                    </div>
                                ) : section.content && (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                        {section.items.map((it: any, idx: number) => (
                                            <div key={idx} className="p-5 rounded-[16px] bg-bg-surface/50 border border-border-light flex flex-col gap-3">
                                                ...
                                                <div className="text-text-primary text-[16px] font-bold leading-tight">{it.타이틀}</div>
                                                <p className="text-text-secondary/70 text-[14px] leading-relaxed break-keep font-medium">{it.설명}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                    ...
                                    <div className="flex flex-col gap-8 mt-4">
                                        {section.quotes.map((q: any, idx: number) => (
                                            <div key={idx} className="relative pl-5 border-l-2 border-brand-primary/40">
                                                <div className="text-[16px] font-medium text-white/80 leading-relaxed mb-3 break-keep">
                                                    {q.text}
                                                </div>
